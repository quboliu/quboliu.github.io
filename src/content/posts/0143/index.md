...
modDatetime: 2026-09-08T15:55:34+08:00
...
> 暂时想到的是这些，有继续想到再补充
>
> ——[楼主 wKong753900，主帖](https://v2ex.com/t/1240266)

> [!QUOTE] 楼主补充：现有软件架构（第 1 楼）
> 软件架构上主要有自建的流媒体服务，P2P 使用的 turn 服务，Go 微服务后端加各种中间件集群(Minio 集群，Pgsql 集群，Redis 集群，Mqtt 集群，RocketMQ 集群等等)
>
> ——[第 1 楼](https://v2ex.com/t/1240266#r_18060370)

> [!QUOTE] 楼主补充：设备规模与存储计划（第 13 楼）
> 一年出货量大概 100 万台设备吧，按 10%日活，前期是打算用 OSS 的，后面量上来了得自己搭
>
> ——[第 13 楼](https://v2ex.com/t/1240266#r_18060609)

> [!NOTE] 楼主在评论中的其余背景与疑问（按楼层整理，非逐字引文）
> - **统一入口（第 3 楼）：** 追问账户数据如何同步、跨区访问是否需要统一入口，以及 GeoDNS 是否适合承担这个入口。[出处](https://v2ex.com/t/1240266#r_18060426)
> - **回放与缓存（第 6 楼）：** 媒体访问中有大量回放；已考虑边缘网关缓存，但仍不确定跨区如何处理。他也担心按数据类型区分职责会要求把服务拆得很细。[出处](https://v2ex.com/t/1240266#r_18060474)
> - **跨区频率（第 7 楼）：** 怀疑跨区用户只是少数，大部分用户会集中在一个地区，因此在权衡是否值得为跨区场景增加复杂度。这是待验证的判断，不是已有统计。[出处](https://v2ex.com/t/1240266#r_18060490)
> - **隐私准备（第 14 楼）：** 已想到隐私和地区要求，但具体落地方式尚不清楚；当时也在询问评论中“全球区”的含义，仍把数据同步视为主要难点。[出处](https://v2ex.com/t/1240266#r_18060637)
> - **非洲低端机型（第 15 楼）：** 有一款重点销往非洲的设备，芯片能力有限，采用自定义传图方式而非标准媒体流，这也是自研接入协议的原因。楼主称单台售价不足“10 块钱”，未说明币种。[出处](https://v2ex.com/t/1240266#r_18060655)
> - **供应商调研（第 18 楼）：** 同时寻找价格合适的服务器厂商，并确认海外对象存储能否用于非标准媒体数据。[出处](https://v2ex.com/t/1240266#r_18060755)
> - **团队经验（第 22 楼）：** 楼主表示缺少海外架构经验，容易把注意力集中在某一个技术问题上。[出处](https://v2ex.com/t/1240266#r_18061040)
> - **部署与范围（第 27 楼）：** 倾向按地区部署；非标准设备仍是兼容性顾虑；本次暂不处理中国境内与境外系统互联，并准备继续查看供应商信息。[出处](https://v2ex.com/t/1240266#r_18061180)
> - **带宽与商业成本（第 33 楼）：** 认为海外带宽成本较高，云存储服务的费用最终需要在商业模式中得到覆盖。[出处](https://v2ex.com/t/1240266#r_18062037)
>
> 以上集中呈现楼主提供的背景、估计、计划和疑问；后文再区分哪些是需求，哪些需要验证，哪些方案应当调整。本次已核对至网页第 42 楼；楼主最新的需求方向变化见下方说明。

> [!IMPORTANT] 最新进展：楼主已调整方案方向（第 42 楼）
> 楼主表示，已结合讨论和查到的合规资料重新制定方案，不再围绕全球化问题纠结。他没有公布新方案，也未明确说明是否取消同账号跨区使用。
>
> 因此，下文保留原始问题与参考设计，但将跨区账号体验 R1 改为待重新确认的条件项。先确定各市场的数据与访问边界，再决定是否需要全球身份服务。[第 42 楼](https://v2ex.com/t/1240266#r_18062712)

一家硬件公司准备建设自己的海外软件平台：北美注册的用户到了非洲，应该继续使用原账号；设备要上传图片和视频；公司又承担不起把整套服务和所有媒体复制到每个地区的成本。

这是 [V2EX 主题 1240266](https://v2ex.com/t/1240266#reply34) 提出的问题。本文把主帖作为需求材料，把评论作为方案评审意见，走一遍从需求分析到概要设计、验证和上线的过程。资料截点为 2026 年 9 月 8 日，已更新至网页可见的第 1–42 楼。评论作者没有参与本文的编写，也不代表认可最终方案。

**在保留跨区账号体验的前提下，本文的参考方案是：租户固定归属区、区内单写、跨区访问原归属区、媒体按需分发。** 同一个账号出国后继续使用原数据，靠的是稳定身份和归属区路由，不必把用户表、业务库和录像复制到所有国家。
...
**这是一份基于原始需求的条件性方案定稿，不是楼主已采用方案的披露。** 原帖没有流量样本、报价、完整设备协议和客户合同；因此性能、费用和恢复目标均标明假设与验证条件，不当作已经具备的生产能力。
...
| F9 | 楼主已结合合规资料重订方案，不再围绕全球化纠结 | [#42](https://v2ex.com/t/1240266#r_18062712)；新方案与跨区账号需求是否保留均未公布 |

### 1.3 需求基线

P0 为本参考方案的试点要求，P1 为后续完善项。新增 F9 后，R1 单列为待确认条件项；若保留，才进入试点范围并执行 T1。对应验收用例见第四部分。

| ID | 优先级 | 行为要求 | 来源/性质 | 验收 |
|---|---|---|---|---|
| R1 | 条件项 | 若仍要求跨区账号体验：已注册账号跨国后能登录并访问原租户设备，不重复注册 | F3 原始诉求；F9 后待重确认 | T1（启用 R1 时） |
...
| R6 | P0 | 凭据、私有媒体、运维访问和备份受地区政策控制 | #10、#35–37、#41 提出的风险，经设计具体化 | T6、T7 |
...
评论楼层依据[原帖网页](https://v2ex.com/t/1240266)，评审覆盖截至本次更新可见的全部 42 楼；第 38–42 楼集中评审于 2.7 节。#34 是对 #4/#16 的赞同，没有新增需求。
...
| 多区完全独立账号 | 故障和数据边界简单 | 无法满足原始 R1 | 保留 R1 时不采用；取消 R1 时可重评 |
...
### 2.7 新增回复：补充经验，以及需求方向的变化

| 楼层 | 新信息 | 对本文的影响 |
| --- | --- | --- |
| [#38](https://v2ex.com/t/1240266#r_18062351) | 旁观讨论 | 无新增事实或方案 |
| [#39](https://v2ex.com/t/1240266#r_18062353) | 分享统一注册/开通入口、选区、同步账号、登录后转向归属站点的实践 | 属于 2.4 中的“全局账户中心、区域业务库”候选，说明它有实际使用案例；不能证明本项目需要它 |
| [#40](https://v2ex.com/t/1240266#r_18062370) | 分享使用 Azure Blob 的经验，建议托管存储 | 支持托管存储方向；没有本项目的报价、链路和合同证据，不足以把 AWS 基线改成 Azure |
| [#41](https://v2ex.com/t/1240266#r_18062519) | 提醒不同数据类型有不同存储位置、方式和保留要求 | 将既有 R6/T7 细化成逐类数据清单，避免只以“部署在欧洲”验收 |
| [#42](https://v2ex.com/t/1240266#r_18062712) | 楼主本人已根据讨论与合规资料重订方案，调整了对全球化的关注 | 新增 F9，将 R1 从 P0 改为待确认条件项；不推断楼主采用了本文方案，也不推断跨区访问已被禁止 |

#39 的统一入口与本文的区域直连都能成立，选择取决于产品要求。若只需要租户自己的用户在外地继续登录，组织码加固定归属区已足够；若必须提供统一账号发现，再评估账户中心。每次登录经过主站还会增加依赖，异步同步账号也需要处理刚注册就访问其他站点的情况。

#41 对应的实施补充是：为账号/凭据、设备标识、媒体原件、派生物、访问日志及备份，逐项记录用途、存储地、可访问者、保留期、删除方式和跨区处理条件，由相应责任人核准。这份清单是部署与 T7 的输入，而不是一句泛化的地区合规结论。

**本次不改区域单写、托管存储和按政策分发的架构骨架，改的是需求基线的确定程度。** 若业务保留 R1，继续使用本文的归属区访问流程；若不保留，一期可按区域独立开通账号，省去统一账号发现和跨区入口兼容。无论哪种情况，都不能仅凭这条回复决定媒体可以跨区缓存或海外运维可以访问哪些数据。
...
| 用例 | 方法 | 通过标准 | 对应需求 |
|---|---|---|---|
| T1 跨区登录（启用 R1 时） | 美国租户在非洲探测端登录、读取及上传；新手机使用组织码 | 身份和租户不变，无新账号，权限正确 | R1 |
...
| 2 一个区域纵向打通 | 邀请登录→绑定→上传→回放→删除；监控与基础设施模板 | T3/T5/T6 通过，启用 R1 时另通过 T1；故障不误报保存 |
...
产品负责人首先依据 F9 重确认 R1，再维护需求和套餐；技术负责人维护 ADR 与数据不变量；设备负责人维护协议/固件兼容矩阵；后端负责幂等、权限和任务补偿；SRE 负责容量、部署、值班、恢复演练；测试负责人负责验收证据；隐私负责人核定地区数据流；财务负责预算与单位经济性。同一个人可兼任，但每一项必须有人负责。
