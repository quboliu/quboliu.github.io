...
modDatetime: 2026-09-11T16:42:06+08:00
...
## 7. 补充：事件为什么需要全序，又为什么全序是瓶颈

第 13 章在讨论事件排序时回到这个概念：「从形式化角度看，为事件决定全序称为全序广播……这等价于共识。」[DDIA 第 13 章](https://learning.oreilly.com/library/view/designing-data-intensive-applications/9781098119058/ch13.html) 这句话可以按本文的框架再拆一层。

**事件天然只有偏序。** 因果关系（happens-before）只约束有依赖的事件对：A 导致 B，则 A 在前。两个互不相关的并发事件——例如两个用户同时操作——在因果上没有定义顺序。偏序意味着不同节点可以合法地以不同顺序看到同一批事件：两个服务各自产生的事件，或离线客户端后同步的事件，都属于这种情况。多数时候这无所谓，但有些场景顺序决定结果：先删好友再发通知，与先发通知再删好友，是两种结局。

**「决定全序」就是本文所述的那套机制。** 让所有节点对任意两个事件的先后达成一致，把偏序人为提升为全序——这正是全序广播的「全序交付」性质提供的服务，也正是共识。所以第 13 章那句话不是新定义，而是同一个等式在事件语境下的复述。

**代价在于瓶颈。** 第 13 章随即指出：大多数共识算法假设单个节点的吞吐量足以处理整个事件流，没有让多个节点分担排序工作的机制。换句话说，「决定全序」必须收敛到逻辑上的单点——本文例子里的排号节点 S，或 Raft 的领导者。事件量一旦超过单点能力，全序本身就成了瓶颈。

**因此第 13 章给出绕过全序的替代方向**，各有限制：

- **逻辑时间戳**：无需协调即可提供全序，但接收方要自己处理乱序到达的事件，且事件必须携带额外元数据；
- **记录因果依赖**：给事件分配唯一标识，后续事件引用它来表达依赖，把「全序」降级为只记录真正重要的偏序边；
- **冲突解决算法**：能消化意外顺序到达的事件，适合维护状态，但对已发生的外部副作用（通知已经发出去了）无能为力。

一句话收束：**全序广播回答「怎样让所有节点认同同一个顺序」，第 13 章追问的是「这个顺序值不值得要」——值得时，它是共识；不值得时，用因果元数据代替全序，省下那个单点瓶颈。**
