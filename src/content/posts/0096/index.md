...
**The Google File System｜Google 文件系统**
...
### Categories and Subject Descriptors｜分类与主题描述词
...
**Figure 1: GFS Architecture｜图：GFS 架构。**
...
**Table 1: File Region State After Mutation｜表：变更之后的文件区域状态。**
...
**Figure 2: Write Control and Data Flow｜图：写入的控制流与数据流。**
...
### 4.3 Creation, Re-replication, Rebalancing｜创建、重新复制与再平衡
...
### 4.5 Stale Replica Detection｜陈旧副本检测
...
#### 5.1.3 Master Replication｜主控节点复制
...
**Table 2: Characteristics of two GFS clusters｜表：两个 GFS 集群的特征。**
...
**Table 3: Performance Metrics for Two GFS Clusters｜表：两个 GFS 集群的性能指标。**
...
#### 6.3.1 Methodology and Caveats｜方法与注意事项
...
**Table 4: Operations Breakdown by Size (%). For reads, the size is the amount of data actually read and transferred, rather than the amount requested.｜表：按大小划分的操作构成（%）。对于读取，大小是实际读取并传输的数据量，而非请求量。**
...
#### 6.3.2 Chunkserver Workload｜块服务器工作负载
...
**Table 5: Bytes Transferred Breakdown by Operation Size (%). For reads, the size is the amount of data actually read and transferred, rather than the amount requested. The two may differ if the read attempts to read beyond end of file, which by design is not uncommon in our workloads.｜表：按操作大小划分的传输字节构成（%）。对于读取，大小是实际读取并传输的数据量，而非请求量。如果读取试图越过文件末尾，二者可能不同；按设计，这在我们的工作负载中并不少见。**
...
#### 6.3.3 Appends versus Writes｜追加与写入的对比
...
**Table 6: Master Requests Breakdown by Type (%)｜表：按类型划分的主控节点请求构成（%）。**
...
#### 6.3.4 Master Workload｜主控节点工作负载
