...
modDatetime: 2026-09-08T16:47:20+08:00
...
**CAS 让所有节点读取同一个获胜提议；共享日志让所有节点读取同一个排在首位的提议；取值并加一只让各节点知道自己的名次，不能普遍地让它们知道第一名是谁。**

CAS 只有一次能把空值改成提议，改进去的就是共同决定。共享日志给提议排出共同顺序，约定第一条为共同决定；不必等所有提议到齐，第一条一经确定，后来者就不能插到它前面。

各节点未必同时读到结果，但读到的确定结果必须相同，也不依赖获胜者亲自通知。取值并加一缺少的是可查询的“名次—提议”对应关系；如果由中心发号时一并保存并公开它，就能补上这项能力。第五节具体讨论这个增强方案。
...
*图中的取号结果分别属于各节点本地，不会自动共享；右栏指裸计数器，不包括第五节新增的中心记录与查询服务。第五节给出其他节点停止后无法分辨胜者的具体历史。*
...
**能。** 让中心 S 接收完整提议，分配号码，同时保存可查询的对应关系：
...
因此，你提出的办法成立；它解决问题靠的是新增的原子记录与查询能力。某个发号器内部即使用共识实现，只要没有向调用者开放这些能力，也不能仅凭内部实现推断它的接口具有相同能力。
