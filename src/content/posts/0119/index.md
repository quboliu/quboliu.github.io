...
tags:
  ...
  - "Multi-Paxos"
  - "Raft"
description: "canonical《Paxos的魔法学研究报告》全文转载，保留原文5幅配图，并附GPT-5.6-sol评价摘要和原文链接。"
---

> **转载说明**
>
> - 原作者：canonical
> - 原文：[知乎《Paxos的魔法学研究报告》](https://zhuanlan.zhihu.com/p/193117183)
> - 完整性依据：[作者仓库中的 Markdown 源文件](https://github.com/entropy-cloud/nop-entropy/blob/c8739a5c544b1e0ea1d4ee3d644ce71c62b6a030/docs/theory/paxos.md)
>
> 本文以作者公开的 Markdown 源文件为正文校对依据，并按知乎原文补齐文末的 NopPlatform 项目链接，保留全部正文和 5 幅原图。除本段转载说明和下面的评价摘要外，没有增删原文内容。

## GPT-5.6-sol 评价摘要

以下评价摘要来自 OpenAI GPT-5.6-sol。

这是一篇风格很鲜明的技术随笔，适合在已经知道 Paxos 基本步骤之后，用来补充直觉。文章最有价值的地方，是用“时间静止”“僵尸 Leader”“对称破缺”等说法，把 ballot、fencing、Quorum 交集，以及“值已经被选定”和“节点已经知道结果”的区别串了起来。第五、六节尤其值得读。

不过，它不适合单独当作 Paxos 的实现说明。ProposalID 是由 Proposer 选择的，Phase 1 通常只需要多数派响应；所谓“时间静止”，准确地说只是 Acceptor 没有见过更高的 ballot，并不表示系统真的停止了并发；Leader 本地写下但尚未提交的日志，也还不是不可改变的结果。把这些边界记住，再配合 [Paxos Made Simple](https://www.microsoft.com/en-us/research/publication/paxos-made-simple/) 阅读，这篇文章会更有帮助。
...
![paxos\_consensus](./paxos_consensus.png)
...
![paxos\_consensus\_fail](./paxos_consensus_fail.png)
...
![paxos\_phase2](./paxos_phase2.png)
...
来看一个Grid Quorum的例子，
![paxos\_grid\_quorum](./paxos_grid_quorum.png)
