...
- 树解决的问题：希望以一种“折中”的方式来同时满足一定效率下的**动态增删**和**静态改查**。
...
- **总结：这边的重点都是放在平衡上。原因在下面的板子上：**
  > **平衡的树是N点节点所能组成的高度最低的树。**
  >
  > **总结：** 一层只搜索一个节点，因此查找效率直接受树高影响。
...
- **双分支的删除**
  ...
  - **方案 A**：以后继 $e_2$ 代替被删除节点。

  - **方案 B**：以前驱 $e_0$ 代替被删除节点。
...
2.  后继节点：
    ...
    - 在二叉搜索树中，给定节点的后继节点是键值比该节点大的最小节点。

      - 如果该节点有右子树，那么后继节点是其右子树中的最小节点。
