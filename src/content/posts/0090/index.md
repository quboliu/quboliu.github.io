...
**Distributed Snapshots: Determining Global States of Distributed Systems｜分布式快照：确定分布式系统的全局状态**
...
## 2. MODEL OF A DISTRIBUTED SYSTEM｜分布式系统模型
...
**Fig. 1. A distributed system with processes $p$, $q$, and $r$ and channels $c1$, $c2$, $c3$, and $c4$.｜图。一个包含进程 $p$、$q$、$r$ 以及信道 $c1$、$c2$、$c3$、$c4$ 的分布式系统。**
...
**Fig. 2. The simple distributed system of Examples 2.1 and 2.2.｜图。例 2.1 和例 2.2 中的简单分布式系统。**
...
**Fig. 3. State-transition diagram of a process in Example 2.1.｜图。例 2.1 中一个进程的状态转移图。**
...
**Fig. 4. Global states and transitions of the single-token conservation system.｜图。单令牌守恒系统的全局状态及其转移。**
...
**Fig. 5. State-transition diagram for process $p$ in Example 2.2.｜图。例 2.2 中进程 $p$ 的状态转移图。**

> **图表中文解读：** $p$ 的初始状态为 $A$；发送 $M$ 时由 $A$ 转到 $B$，接收 $M'$ 时由 $B$ 转回 $A$。

![State-transition diagram for process q in Example 2.2.](./fig-06.png)

**Fig. 6. State-transition diagram for process $q$ in Example 2.2.｜图。例 2.2 中进程 $q$ 的状态转移图。**

> **图表中文解读：** $q$ 的初始状态为 $C$；发送 $M'$ 时由 $C$ 转到 $D$，接收 $M$ 时由 $D$ 转回 $C$。

![A computation for Example 2.2.](./fig-07.png)

**Fig. 7. A computation for Example 2.2.｜图。例 2.2 的一次计算。**
...
### 3.1. Motivation for the Steps of the Algorithm｜算法步骤的动机
...
### 3.2 Global-State-Detection Algorithm Outline｜全局状态检测算法纲要
...
### 3.3 Termination of the Algorithm｜算法的终止
...
## 4. PROPERTIES OF THE RECORDED GLOBAL STATE｜所记录全局状态的性质
...
**Fig. 8. A recorded global state for Example 2.2.｜图。例 2.2 中记录所得的一个全局状态。**
