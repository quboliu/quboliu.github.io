...
tags:
  ...
  - "resourceVersion"
  - "分布式系统"
description: "Kubernetes API Concepts 官方文档中英对照精读：覆盖资源 URI、list/watch、分页、内容协商、更新、试运行与 resourceVersion 语义。"
---
...
You can view the [API reference](https://kubernetes.io/docs/reference/kubernetes-api/) online,
...
>
> 你可以在线查看 [API 参考](https://kubernetes.io/zh-cn/docs/reference/kubernetes-api/)，
> 或继续阅读以了解 API 的一般信息。

## Kubernetes API terminology｜Kubernetes API 术语
...
> 大多数 Kubernetes API
> 资源类型都是[对象](https://kubernetes.io/zh-cn/docs/concepts/overview/working-with-objects/kubernetes-objects/#kubernetes-objects)：
...
see the [API reference](https://kubernetes.io/docs/reference/kubernetes-api/) for more information. It
...
>
> 取决于对象是什么，每个子资源所支持的动词有所不同 - 参见 [API 文档](https://kubernetes.io/zh-cn/docs/reference/kubernetes-api/)以了解更多信息。
> 跨多个资源来访问其子资源是不可能的 - 如果需要这一能力，则通常意味着需要一种新的虚拟资源类型了。

## Efficient detection of changes｜高效检测变更
...
Provided that the `WatchList` [feature gate](https://kubernetes.io/docs/reference/command-line-tools-reference/feature-gates/)
...
[`BOOKMARK` event](https://kubernetes.io/docs/reference/using-api/api-concepts/#watch-bookmarks)
...
> 如果启用了 `WatchList` [特性门控](https://kubernetes.io/zh-cn/docs/reference/command-line-tools-reference/feature-gates/)，
...
> 则继续发送 [`BOOKMARK` 事件](https://kubernetes.io/zh-cn/docs/reference/using-api/api-concepts/#watch-bookmarks)。
...
[collections](https://kubernetes.io/docs/reference/using-api/api-concepts/#collections).
...
> 特别是对于大型资源或[集合](https://kubernetes.io/zh-cn/docs/reference/using-api/api-concepts/#collections)，
...
## Retrieving large results sets in chunks｜分块检视大体量结果
...
has `kind` set to
[`ServiceList`](https://kubernetes.io/docs/reference/kubernetes-api/service-resources/service-v1/#ServiceList); each item in that collection represents a single Service. For example:
...
> [`ServiceList`](https://kubernetes.io/zh-cn/docs/reference/kubernetes-api/service-resources/service-v1/#ServiceList)；
...
[Kubernetes API](https://kubernetes.io/docs/reference/kubernetes-api/) documentation.
...
> 你可以从 [Kubernetes API](https://kubernetes.io/zh-cn/docs/reference/kubernetes-api/) 文档中获取有关每种集合类型的更多信息。
>
...
## Receiving resources as Tables｜以表格形式接收资源
...
[extends the core Kubernetes API](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/)
...
> [扩展核心 Kubernetes API](https://kubernetes.io/zh-cn/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/)
...
## Alternate representations of resources｜资源的其他表示形式
...
See the Kubernetes [API reference](https://kubernetes.io/docs/reference/kubernetes-api/) for a list of
...
>
> 有关每个 API 支持的内容类型列表，请参阅 Kubernetes [API 参考](https://kubernetes.io/zh-cn/docs/reference/kubernetes-api/)。
>
> 例如：
...
### Kubernetes Protobuf encoding｜Kubernetes Protobuf 编码
...
### Validation for unrecognized or duplicate fields｜检查无法识别或重复的字段
...
### Make a dry-run request｜发起试运行请求
...
> 当你设置 `?dryRun=All` 时，将运行任何相关的准入控制器，
...
declare in their
[configuration object](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#validatingwebhook-v1-admissionregistration-k8s-io)
...
> 所有内置准入控制插件都支持试运行。
> 此外，准入 Webhook 还可以设置[配置对象](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#validatingwebhook-v1-admissionregistration-k8s-io)
...
* `UID`: [uniquely identifies](https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#uids) the object and is randomly generated (non-deterministic)
...
> * `UID`：[唯一标识](https://kubernetes.io/zh-cn/docs/concepts/overview/working-with-objects/names/#uids)对象，
>   取值随机生成（非确定性）
> * `resourceVersion`：跟踪对象的持久化（存储）版本
> * 变更性准入控制器所设置的字段
> * 对于 `Service` 资源：`kube-apiserver` 为 `Service` 对象分配的端口和 IP 地址

### Dry-run authorization｜试运行的授权
...
See [Authorization Overview](https://kubernetes.io/docs/reference/access-authn-authz/authorization/).

> 参阅[鉴权概述](https://kubernetes.io/zh-cn/docs/reference/access-authn-authz/authorization/)以了解鉴权细节。

## Updates to existing resources｜更新现有资源
...
: Server Side Apply YAML (a Kubernetes-specific extension, based on YAML).
  ...
  media type. See [Server Side Apply serialization](https://kubernetes.io/docs/reference/using-api/server-side-apply/#serialization)
  for more details.
...
>   更多细节参阅[服务器端应用序列化](https://kubernetes.io/zh-cn/docs/reference/using-api/server-side-apply/#serialization)。
...
See [Server Side Apply](https://kubernetes.io/docs/reference/using-api/server-side-apply/) for more details.

> Kubernetes 的[服务器端应用](https://kubernetes.io/zh-cn/docs/reference/using-api/server-side-apply/)功能允许控制平面跟踪新创建对象的托管字段。
...
>
> 更多细节参阅[服务器端应用](https://kubernetes.io/zh-cn/docs/reference/using-api/server-side-apply/)。

### Choosing an update mechanism｜选择更新机制

#### HTTP PUT to replace existing resource｜HTTP PUT 替换现有资源 {#update-mechanism-update}
...
#### HTTP PATCH using JSON Patch｜使用 JSON Patch 的 HTTP PATCH
...
#### HTTP PATCH using Server-Side Apply｜使用服务器端应用的 HTTP PATCH
...
### `resourceVersion` fields in metadata｜metadata 中的 `resourceVersion`
...
[v1.meta/ObjectMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#objectmeta-v1-meta) - The `metadata.resourceVersion` of a resource instance identifies the resource version the instance was last modified at.

[v1.meta/ListMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#listmeta-v1-meta) - The `metadata.resourceVersion` of a resource collection (the response to a **list**) identifies the resource version at which the collection was constructed.

> 客户端在资源中查找资源版本，这些资源包括来自用于 **watch** 的响应流资源，或者使用 **list** 枚举的资源。
>
> [v1.meta/ObjectMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#objectmeta-v1-meta) -
> 资源的 `metadata.resourceVersion` 值标明该实例上次被更改时的资源版本。
>
> [v1.meta/ListMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#listmeta-v1-meta) - 资源集合即
> **list** 操作的响应）的 `metadata.resourceVersion` 所标明的是 list 响应被构造时的资源版本。

### `resourceVersion` parameters in query strings｜查询字符串中的 `resourceVersion` 参数
...
### Semantics for **get** and **list**｜**get** 和 **list** 语义
...
[list metadata](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#listmeta-v1-meta) of the collection as
well as [object metadata](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#objectmeta-v1-meta)
...
> 当你 **list** 资源并收到集合响应时，
> 响应包括集合的[列表元数据](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#listmeta-v1-meta)。
> 以及该集合中每个项目的[对象元数据](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#objectmeta-v1-meta)。
...
### "410 Gone" responses｜"410 Gone" 响应
...
### Unavailable resource versions｜不可用的资源版本
