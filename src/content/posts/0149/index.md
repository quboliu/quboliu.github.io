...
![单向通信可能反复提高任期；预投票把正式任期提升推迟到获得多数预票之后。](./prevote-election-flow.png)
...
节点超时后，先增加正式任期，再请求选票，最后才知道能否得到多数支持。

预投票路径变成：

节点超时后先试探多数支持，正式任期保持不变。只有拿到多数预票，才增加正式任期并发起真正选举；真正赢得选票后才能当选。
