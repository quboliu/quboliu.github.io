...
| 假设链路速率 | 不扣开销的字节速率 | 传输 1 GB 的理论最短序列化时间 |
| --- | --- | --- |
| 1 Gb/s | 125 MB/s | 8 s |
| 10 Gb/s | 1.25 GB/s | 0.8 s |
| 100 Gb/s | 12.5 GB/s | 0.08 s |
...
| 源地域 → 目标地域 | P50 RTT | 可建立的尺度感 |
| --- | --- | --- |
| East US → East US 2 | 8 ms | 相近地域也需要毫秒级往返 |
| East US → West US | 69 ms | 横跨美国的这个地域对，约数十毫秒 |
| Japan East → West US | 107 ms | 这个跨太平洋地域对，超过百毫秒 |
| South Africa North → West US | 273 ms | 这个长距离地域对，达到数百毫秒 |
...
| 相同吞吐，平均停留升至 200 ms | 平均在途 2,000 个请求 | 同样的流量可以占用更多缓冲区和上下文 |
