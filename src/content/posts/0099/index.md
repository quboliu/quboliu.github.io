---
lang: "zh-CN"
pubDatetime: 2025-07-20T12:00:00+08:00
modDatetime: 2026-08-10T15:33:40+08:00
timezone: "Asia/Shanghai"
title: "论文阅读 | Unreliable Failure Detectors for Reliable Distributed Systems｜可靠分布式系统中的不可靠故障检测器"
...
tags:
  ...
  - "原子广播"
  - "容错"
description: "按语义单元编排的中英对照精读 Chandra 与 Toueg 的经典论文，系统梳理不可靠故障检测器的完备性、准确性与可归约性，以及它们对共识、原子广播可解性和容错边界的影响。"
---

**Unreliable Failure Detectors for Reliable Distributed Systems｜可靠分布式系统中的不可靠故障检测器**
...
### 2.1 Failures and failure patterns｜故障与故障模式
...
### 2.3 Failure detector properties｜故障检测器属性
...
### 2.4 Failure detector classes｜故障检测器类别
...
Fig. 1. Eight classes of failure detectors defined in terms of accuracy and completeness.｜图. 按准确性与完备性定义的八类故障检测器。
...
Fig. 2. Transforming D into D′.｜图. 将 D 转换为 D′。
...
Fig. 3. $T_{D\to D'}$: From Weak Completeness to Strong Completeness.｜图. $T_{D\to D'}$：从弱完备性到强完备性。
...
## 3. FROM WEAK COMPLETENESS TO STRONG COMPLETENESS｜从弱完备性到强完备性
...
Fig. 4. Reliable Broadcast by message diffusion.｜图. 通过消息扩散实现可靠广播。
...
## 6. SOLVING CONSENSUS USING UNRELIABLE FAILURE DETECTORS｜使用不可靠故障检测器求解共识
...
### 6.1 Solving Consensus using S｜使用 S 求解共识
...
Fig. 5. Solving Consensus using any D ∈ S.｜图. 使用任意 D ∈ S 求解共识。
...
### 6.2 Solving Consensus using ◇S｜使用 ◇S 求解共识
...
Fig. 6. Solving Consensus using any D ∈ ◇S.｜图. 使用任意 D ∈ ◇S 求解共识。
...
### 6.3 A lower bound on fault-tolerance｜容错能力的下界
...
### 7.1 Reducing Atomic Broadcast to Consensus｜将原子广播归约为共识
...
Fig. 7. Using Consensus to solve Atomic Broadcast.｜图. 使用共识求解原子广播。
...
## 8. COMPARING FAILURE DETECTOR CLASSES｜比较故障检测器类别
...
Fig. 8. Comparing the eight failure detector classes by reducibility.｜图. 按可归约性比较八类故障检测器。
...
Fig. 9. Problem solvability in different distributed computing models.｜图. 不同分布式计算模型中的问题可解性。
...
Fig. 10. A time-out based implementation of D ∈ ◇P in models of partial synchrony.｜图. 在部分同步模型中基于超时实现 D ∈ ◇P。
...
### 9.2 Unreliable failure detection in shared memory systems｜共享内存系统中的不可靠故障检测
...
### A hierarchy of failure detector classes and bounds on fault-tolerance｜故障检测器类别层次与容错能力界限
...
Fig. 11. Classes of repentant failure detectors ordered by reducibility. For each class C, the maximum number of faulty processes for which Consensus can be solved using C is given.｜图. 按可归约性排列的悔改型故障检测器类别；图中给出使用各类别求解共识时所能容忍的最大故障进程数。
