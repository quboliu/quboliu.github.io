# 数据库复制日志到底在复制什么：从 DDIA 到 PostgreSQL WAL 与 MySQL binlog
...
本地[第 6 章源文件](chapters/ch06-replication.typ)在“复制日志的实现”下依次讨论：
...
![同一条更新在语句、物理和行级逻辑复制中的三种表示](replication-logs/assets/01-taxonomy.png)
...
![PostgreSQL WAL 的段、页、记录和存储操作](replication-logs/assets/02-pg-wal-v2.png)
...
![PostgreSQL 物理复制协议和备库重放时序](replication-logs/assets/03-pg-physical.png)
...
![PostgreSQL 从 WAL 解码到 pgoutput，再应用到订阅端](replication-logs/assets/04-pg-logical-v2.png)
...
![PostgreSQL 和 MySQL 的日志来源与消费路径对比](replication-logs/assets/07-two-engines.png)
...
![InnoDB redo 的文件、日志块和页操作](replication-logs/assets/09-innodb-redo.png)
...
![MySQL binlog 的事件头与 STATEMENT／ROW 内容对比](replication-logs/assets/05-mysql-binlog.png)
...
![PostgreSQL pgoutput 与 MySQL Update_rows 的真实字节对照](replication-logs/assets/08-real-bytes-v2.png)
...
![MySQL binlog 传输、relay log 和应用线程](replication-logs/assets/06-mysql-stream.png)
...
- [PostgreSQL 完整 SQL 会话与两种解码结果](replication-logs/evidence/postgres-session.txt)
- [PostgreSQL 实测 WAL 重做记录](replication-logs/evidence/postgres-waldump.txt)
- [PostgreSQL pgoutput 原始十六进制](replication-logs/evidence/postgres-pgoutput.hex)
- [MySQL SHOW BINLOG EVENTS 实测事件序列](replication-logs/evidence/mysql-show-binlog-events.txt)
- [MySQL STATEMENT 原始 binlog](replication-logs/evidence/mysql-statement.binlog)
- [MySQL ROW/FULL 原始 binlog](replication-logs/evidence/mysql-row-full.binlog)
- [MySQL ROW/MINIMAL 原始 binlog](replication-logs/evidence/mysql-row-minimal.binlog)
- [FULL 事件十六进制](replication-logs/evidence/mysql-row-full-hex.txt)与[MINIMAL 事件十六进制](replication-logs/evidence/mysql-row-minimal-hex.txt)
- [实验说明与复现 SQL](replication-logs/README.md)
- [Image Gen 配图提示词](replication-logs/figure-prompts.json)
